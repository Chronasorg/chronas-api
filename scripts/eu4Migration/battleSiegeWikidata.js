...
// const folderDir = resolve("../../../dataToMigrate/wars")
const folderDir = resolve("../../dataToMigrate/wars") // extendedtimeline // imperium_universalis
...
handleWarFile = (fileName) => new Promise((resolve, reject) => {
  ...
  LineReader.eachLine(folderDir + '/' + fileName, function(line) {
    ...
    utils.getEnWikiBySearch(entityName)
      .then((preWiki) => {
        console.debug("preWiki",preWiki )
        let enWiki = preWiki ? preWiki : 'war'
        console.debug("imageUrl",entityName, enWiki, 1024 )
...
fs.readdir(folderDir, function( err, files ) {
  ...
  files.reduce(
    (p, x) => p.then(() => {
      c++
      console.debug(files.length,c,"war ", x)
      return handleWarFile(x)
    }),
    Promise.resolve()
  )
