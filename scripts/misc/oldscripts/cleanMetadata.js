...
const rulerBCMap = require("../../eu4Migration/bcRulerMap.json")
...
const provinceList = require('./provinceList.json')
  ...
  //   lineReader.on('line', function (lineno, line) {
    lineReader.eachLine(folderDir + '/provinceHistory.csv', function(line) {
      ...
      if (prev_province && prev_province !==  "Hälsingland" && (!lastWritten.includes(prev_province)) && (province !== prev_province)) {
        ...
        completedArea[prev_province.replace(/\./g, '')] = [rulerBCMap[prev_owner] ? rulerBCMap[prev_owner] : prev_owner,prev_culture,prev_religion,prev_capital,+prev_population]
      ...
      Object.keys(completedArea).forEach((currProv) => {
        if (!completedArea[currProv]) delete completedArea[currProv]
      })

      const cKeys = Object.keys(completedArea)

      const keysToAddEmpty = _.difference(provinceList, cKeys)
      const keysToRemove =  _.difference(cKeys, provinceList)


      keysToAddEmpty.forEach((currProv) => {
        if (currProv === "Lima") console.debug("1Lima")
        completedArea[currProv] = emptyTemplate
        // console.debug(keysToAddEmpty.length,currProv)
      })

      keysToRemove.forEach((currProv) => {
        if (currProv === "Lima") console.debug("2Lima")
        delete completedArea[currProv]
      })

      if (!completedArea["Istria"]) completedArea["Istria"] = completedArea["Görz"]
      if (!completedArea["Balears"]) completedArea["Balears"] = completedArea["Girona"]
      if (!completedArea["Madang"]) completedArea["Madang"] = completedArea["Lae"]
      if (!completedArea["Asmat"]) completedArea["Asmat"] = completedArea["Fak-Fak"]
      if (!completedArea["Chuquiabo"]) completedArea["Chuquiabo"] = completedArea["Macaya"]
      if (!completedArea["Lappi"]) completedArea["Lappi"] = completedArea["Lappland"]
